...
Shader "SHD_Moon"
{
	Properties
	{
		...
		[ASEBegin]_T_Moon("T_Moon", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float2 texCoord29 = packedInput.ase_texcoord1.xy * float2( 0.05,0.2 ) + float2( 0,0 );
				float2 panner31 = ( 1.0 * _Time.y * float2( 0,0.04 ) + texCoord29);
				float4 tex2DNode28 = tex2D( _TextureSample0, panner31 );
				float4 tex2DNode7 = tex2D( _T_Moon, ( texCoord30 + ( tex2DNode28.r * 0.2 ) ) );
				
				surfaceDescription.Color = ( tex2DNode7 * _Float0 ).rgb;
				surfaceDescription.Emission = 0;
				surfaceDescription.Alpha = ( saturate( ( tex2DNode28.r * 10.0 ) ) * tex2DNode7.r * _Alpha );
		...
		Pass
		{
					...
					)
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 texCoord29 = packedInput.ase_texcoord.xy * float2( 0.05,0.2 ) + float2( 0,0 );
				float2 panner31 = ( 1.0 * _Time.y * float2( 0,0.04 ) + texCoord29);
				float4 tex2DNode28 = tex2D( _TextureSample0, panner31 );
		...
		Pass
		{
					...
					)
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 texCoord29 = packedInput.ase_texcoord3.xy * float2( 0.05,0.2 ) + float2( 0,0 );
				float2 panner31 = ( 1.0 * _Time.y * float2( 0,0.04 ) + texCoord29);
				float4 tex2DNode28 = tex2D( _TextureSample0, panner31 );
...
ASEEND*/
//CHKSM=A701D40EBB45291986FBAF7C3175B00D050ABB4D